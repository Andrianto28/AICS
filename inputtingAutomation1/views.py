...
image_folder = str(settings.BASE_DIR) + "/inputtingAutomation1/input_dirr_dif_doc/"
print(image_folder)
...
def ocrnib(request):
    ...
    image_path = image_folder + image_path['nama']
    ...
    if True:
        ...
        ret, gray = cv2.threshold(gray, threshold_value, 255, cv2.THRESH_BINARY)
            ...
            try:
                if 'nama usaha' in text:
                    if re.search(pattern_company, text):
                        dict['company_name'] = re.search(pattern_company, text).group(1)
                    elif re.search(pattern_company_2, text):
                        dict['company_name'] = re.search(pattern_company_2, text).group(1)
                    elif re.search(pattern_company_3, text):
                        dict['company_name'] = re.search(pattern_company_3, text).group(1)
            ...
            try:
                if 'telepon' in text:
                    if re.search(pattern_telephone, text):
                        dict['telephone_number'] = re.search(pattern_telephone, text).group()
            ...
            try:
                if 'kbli' in text:    
                    if re.search(pattern_kbli, text):
                        dict['kbli_number'] = re.search(pattern_kbli, text).group()
        ...
        for text in list:
            try:
                if 'jenis api' in text:
                    dict['api_classification'] = re.search(pattern_api, text).group(1)
        ...
        for text in list:
            try:
                if 'status penanaman modal' in text:
                    dict['investment_status'] = re.search(pattern_investment, text).group(1)
...
def ocrsiup(request):
    ...
    if True:
        ...
        # Perform text detection using Tesseract OCR
        text_siup = pytesseract.image_to_string(img_siup_gray, config= custome_oef_config)
        ...
        for line in lines_siup:
            ...
            try:
                if 'Alamat Perusahaan' in line:
                    ...
                    alamat_perusahaan = ' '.join(lines_siup[index:index_Nama_KBLI])
        ...
        for line in hasil:
            ...
            try:
                line = line.replace(':', '').replace('>', '').replace('|', '').replace('-', '').replace('_','').replace('—','').replace('$','8').strip()
                if '  ' in line:
...
def ocrtdp(request):
    ...
    if True:
        ...
        img_bin_v = cv2.morphologyEx(img_bin_v, cv2.MORPH_OPEN, kernal6v) # keep only vert lines by encoding everything else in vert direction
        ...
        for x,y,w,h,area in stats[2:]:
            if min_size < w*h < max_size:
                ...
                img = cv2.resize(box, None, fx=1.5, fy=1.5, interpolation=cv2.INTER_CUBIC)
            ...
            try: 
                if 'tdp' in line:
                    if re.search(pattern_tdp_number, line):
                        ...
                        line = ocr_results[ocr_results.index(line) + 2].replace('. ','.')
                        dict['tdp_number'] = line
...
def ocrskdp(request):
    ...
    if True:
        ...
        for line in lines_siup:
            try:
                if 'tanggal lahir' in line:
                    place_date_of_birth = line.split('lahir')[1].replace('+','').replace(':','').strip()
        ...
        for line in lines_siup:
            try:
                if 'kelamin' in line:
                    gender = line.split('kelamin')[1].replace('.','').replace('+','').replace(':','').replace('t','l').strip()
                    dict['gender'] = gender
            except:
                dict['gender'] = ''
            ...
            try:
                if 'agama' in line:
                    if 'kewarganegaraan' in line:
                        religion = line.split('kewarganegaraan')[1].replace(':','').replace('isiam', 'islam').strip()
                        if '/' in religion:
                            dict['religion'] = religion.split('/')[0].strip()
                        else:
                            dict['religion'] = religion
                    elif 'agama' in line:
                        religion = line.split('agama')[1].replace(':','').replace('isiam', 'islam').strip()
                        dict['religion'] = religion
            except:
                dict['religion'] = ''
        ...
        for line in lines_siup:
            if 'kewarganegaraan' in line:
                citizenship = line.split('kewarganegaraan')[1].replace('+','').strip()
                if '/' in citizenship:
        ...
        for line in lines_siup:
            ...
            try:
                if re.search(pattern_ktp, line, re.IGNORECASE):
                    id_card_number = line.split('ktp')[1].replace('+','').replace(':','').replace('nomor','').strip() 
                    dict['id_card_number'] = id_card_number
                elif re.search(pattern_nik, line, re.IGNORECASE):
                    id_card_number = line.split('nik')[1].replace('+','').replace(':','').replace('nomor','').strip()
                    dict['id_card_number'] = id_card_number
            except:
                dict['id_card_number'] = ''
        ...
        for line in lines_siup:
            try:
                if 'pekerjaan' in line:
                    occupation = line.split('pekerjaan')[1].replace('+','').strip()
        ...
        for line in lines_siup:
            try:
                if 'nama perusahaan' in line:
                    company_name = line.split('perusahaan')[1].replace(':','').replace(',','').strip()
                    dict['company_name'] = company_name
                elif 'nama usaha' in line:
                    company_name = line.split('usaha')[1].replace(':','').replace(',','').strip()
                    dict['company_name'] = company_name
                elif 'nama perusahan' in line:
                    company_name = line.split('perusahan')[1].replace(':','').replace(',','').strip()
                    dict['company_name'] = company_name
            except:
                dict['company_name'] = ''
        if 'company_name' not in dict:
        ...
        for line in lines_siup:
            try:
                if 'jenis usaha' in line:
                    business_category = ', '.join(lines_siup[lines_siup.index(line):index_alamat_perusahaan]).split('usaha')[1].replace('!','i').replace('2.','').replace('1.','').replace('klasifikasi','').replace('-','').replace('/','').replace(':','').replace('—','').strip()
        ...
        for line in lines_siup:
            try:
                if 'alamat usaha' in line:
                    business_address = line.split('usaha')[1].replace('=','').replace('|','1').strip()
                    dict['business_address'] = business_address
                elif 'alamat perusahaan' in line:
                    business_address = line.split('perusahaan')[1].replace('=','').replace('|','1').strip()
                    dict['business_address'] = business_address
                elif 'tempat usaha' in line:
                    business_address = line.split('usaha')[1].replace('=','').replace('|','1').strip()
                    dict['business_address'] = business_address
                elif line.startswith('domisili perusahaan'):
                    dict['business_address'] = line.split('perusahaan')[1].replace('=','').replace('|','1').strip()
            except:
        ...
        for line in lines_siup:
            try:
                if 'status' in line:
                    building_status = line.split('bangunan')[1].replace('milk', 'milik').replace('sendin','sendiri').strip()
        ...
        for line in lines_siup:
            try:
                if 'akte' in line:
                    establishment_deed_number = line.split('perusahaan')[1].strip()
                    if 'nomor' in establishment_deed_number:
                        dict['establishment_deed_number'] = establishment_deed_number.split('nomor')[1].replace(':','').strip()
                    else:
                        dict['establishment_deed_number'] = establishment_deed_number
                elif line.startswith('akta pendirian perusahaan'):
                    establishment_deed_number = ''.join(lines_siup[lines_siup.index(line):lines_siup.index(line)+3])
                    dict['establishment_deed_number'] = establishment_deed_number.split('perusahaan')[1].replace(':','').replace('notaris','').replace('tanggal','').replace('=','').replace('nomor','').replace('‘','').strip()
            except:
        ...
        for line in lines_siup:
            try:
                if 'kehakiman' in line:
                    judicial_approval_letter = line.split('kehakiman')[1].replace('=','').replace('-','').strip()
        ...
        for line in lines_siup:
            try:
                if 'jawab' in line:
                    if '/' in line:
                        person_in_charge = lines_siup[lines_siup.index(line)+1].split('perusahaan')[1].replace(':','').strip()
    ...
    if allowed_file(image_path):
        ...
        try:
            ...
            if index_terdaftar is not None:
                dict['Adress'] = ' '.join(lines_siup[index_npwp+2:index_terdaftar]).strip()
            ...
            if 'terdaftar' in line:
                ...
                dict['registered'] = terdaftar_line.split(':')[1].replace('/', '-').strip()
